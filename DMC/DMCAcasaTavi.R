...
### this is the code to produce the second DMC report for
...
EOStab <- EOS %>% select(SubjectId, EventDate, EOSREAS, Treatment)
...
varctab <- SO %>% select(SubjectId, SOVARC, Treatment)
...
KU <- KU %>% select(SubjectId, KU1_3, KU1_3CD, KU1_3DAT, KU1_2, KU1_2CD, KU1_2DAT, Treatment)
...
DE <- KU %>% select(SubjectId, KU1_0, KU1_0DAT,Treatment)

DE <- DE %>% filter(KU1_0 == 'Ja')
colnames(DE) <- c('SubjectId', 'Death','Deat date', 'Treatment')
