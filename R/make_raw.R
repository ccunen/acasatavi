...
# Viedoc import: select "Medical coding" + "Event dates". "csv" files and "1 row per activity"
...
raw <- tibble(files = list.files(export_folder)) %>%
  ...
  mutate(data = map(data, ~ .x %>% mutate(across(where(is.character), tolower)) ) ) %>% #added by CC
