...
pcto <-  function(data, var, group, level = 1) {
  var <- ensym(var)
  group <- ensym(group)
  data %>% 
    ...
    mutate(tot = sum(tot),
           pct = round(n/tot*100, digits = 0)) %>% 
    mutate(txt = paste0( pct, "%")) %>% 
...
ae_N <-  function(data, var, group, level = 1) {
  var <- ensym(var)
  group <- ensym(group)
  ...
  data %>%
    ...
    summarise(n_ae = sum(n),
              .groups = "drop_last") %>%
    group_by(!!group) %>%
    mutate(txt = paste0(n_ae)) %>%
    mutate(txt = if_else(n_ae == 0, "0", txt)) %>%
    filter(!!var %in% !!level) %>%
    ungroup %>%
    select(!!group, txt) %>%
    deframe
